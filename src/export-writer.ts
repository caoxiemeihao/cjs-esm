...
function createExportWriter(context: Context) {
  ...
  function write() {
    ...
    if (context.moduleNodes.some((n) => !n.exported && !n.declared)) {
  ...
  function bindToNames() {
    ...
    for (const [name, infos] of allExports) {
      ...
      if (init === 1 && (!declared || infos.lenght === 1)) {
        for (const info of infos) {
          if (info.type === 'name') {
            const node = info.node
            if (node.declared) {
            ...
            info.shareExport = infos.lenght > 1
            writeProperty(info)
          }
        }
      } else {
  ...
  function writeNamedDeclare(node: AcornNodeExt) {
  ...
  function writeNamedExports(node: AcornNodeExt, kind = 'const', shareExport = false) {
  ...
  function writeNestedExports(node: AcornNodeExt) {
  ...
  function writeNamedInit(node, name: string) {
    context.s.appendLeft(
      node.rootPos,
  ...
  function writeDefaultExportDeclare(node: AcornNodeExt, kind: string) {
  ...
  function writeDefaultExportNode(node: AcornNodeExt) {
